...
(cl-defun alist-menu@ (prompt lst &key action (depth 1))
  (cl-labels ((add-props (elem)
				(propertize (format "%s" (car elem)) 'value (last@ elem)))
			  (get-value (selection)
				(get-text-property 0 'value selection))
			  (menu (prompt lst action depth current-depth)
				(let* ((lst (cl-loop for x in lst collect (add-props x)))
					   (selection (if (not action)
									  (ivy-read prompt lst)
									(ivy-read prompt lst :action action)))
					   (value (get-value selection)))
				  (if (and (< current-depth depth) (alistp value))
					  (menu selection value action depth (1+ current-depth))
					value)))
			  (create-fn (fn)
				(if (lambdap fn)
					(eval `(lambda (selection)
							 (funcall ,fn (get-text-property 0 'value selection))))
				  (eval `(lambda (selection)
						   (,fn (get-text-property 0 'value selection))))))
			  (wrap-fn (elem)
				(if (integerp elem)
					elem
				  (list (nth 0 elem)
						(create-fn (nth 1 elem))
						(nth 2 elem)))))
	(let* ((action (if (listp action)
					   (cl-loop for a in action collect (wrap-fn a))
					 (create-fn action))))
	  (menu prompt lst action depth 1))))

;; (defun --caller (value) (message "modified: %s" value))
