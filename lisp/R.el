(class R-workspace-process (terminal-process))
...
(defun R-root-start (&optional buf)
  (interactive)
  (let* ((buf (or buf (current-buffer)))
		 (ws (or (find-buffer-workspace buf) (dirname buf)))
		 (proc (local-config-get :R-workspace-processes ws)))
	(if proc
		proc
	  (progn
		(split-window-vertically)
		(other-window 1)
		(let* ((proc-buf (ansi-term (local-config-get :shell-command)))
			   (proc (get-buffer-process proc-buf))
			   (it (R-workspace-process
					:process proc
					:process-buffer proc-buf
					:cwd ws)))
		  (local-config-set :R-workspace-processes ws it)
		  (process-send-string proc (concat "cd " ws "\n"))
		  (delete-window)
		  (message (concat "Started shell for " ws))
		  it)))))

(defun R-root-buffer-process (&optional buf mapper)
  (when-let* ((buf (or buf (current-buffer)))
			  (it (local-config-get
				   :R-workspace-processes
				   (or (find-buffer-workspace buf)
					   (dirname buf)))))
	(if mapper
		(funcall mapper it)
	  it)))

(defun R-root-live? (&optional buf mapper)
  (cl-labels ((alive? (it) (process-live-p (%. it 'process))))
	(cond
	 ((or (buffer? buf) (eq buf nil))
	  (when-let* ((buf (or buf (current-buffer)))
				  (it (local-config-get
					   :R-workspace-processes
					   (or (find-buffer-workspace buf)
		...
		(if mapper
			(funcall mapper it)
		  it)))
	 ((R-workspace-process-p buf)
	  (when (alive? it)
		(if mapper (funcall mapper it) it))))))

(defun R-root-split (&optional buf direction)
  (interactive)
  (R-root-live?
   buf
   (lambda (it)
	 (when (not (R-root-visible? buf))
	   (with-current-buffer (current-buffer)
		 (if (eq (or direction 'below) 'below)
			 (split-window-below)
		   (split-window-right))
...
(defun R-root-split-right (&optional buf)
  (interactive)
  (R-root-split buf 'right))

(defun R-root-stop (&optional buf)
  (interactive)
  (R-root-live?
   buf
   (lambda (it)
	 (with-slots (process process-buffer) it
	   (if-let* ((window (get-buffer-window process-buffer)))
		   (delete-window window))
	   (kill-process process)
	   (kill-buffer process-buffer)))))

(defun R-root-visible? (&optional buf)
  (interactive)
  (R-root-buffer-process
   buf
   (lambda (it)
	 (with-slots (process-buffer) it
	   (get-buffer-window process-buffer)))))

(defun R-root-hide (&optional buf)
  (interactive)
  (R-root-buffer-process
   buf
   (lambda (it)
